...
class Message(Note):
    def __init__(self, path, line, column, severity, message, checker, fixits=None,
                 notes=None, path_miss = False):
...
class OutputParser(object):
    ...
    extra_note_line_re = re.compile(
        '^(?P<severity>warning|note): '
        '(?P<message>.*)')
    ...
    def _parse_message(self, titer, line):
        ...
        message = Message(
            ...
            match.group('checker').strip(), path_miss = path_miss)
    ...
    def _parse_notes(self, message, titer, line):
        while OutputParser.message_line_re.match(line) is None:
            ...
            if 'severity' in match.groupdict().keys():
                message.notes.append(Note(os.path.abspath(message.path),
                                          int(message.line),
                                          int(message.column),
                                          "",
                                          match.group('message').strip()))
            ...
            line = titer.next()
            if not 'severity' in match.groupdict().keys():
                line = self._parse_code(message, titer, line)

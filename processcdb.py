...
tools = {'clang-tidy': ClangTidy('clang-tidy'),
         'lizard': Lizard('lizard'),
         'cppcheck': CppCheck('cppcheck'),
         'clang': Clang('analyze-build')}

parser = argparse.ArgumentParser(description="Static analysis wrapper",
                                 epilog="Available tools: \n{}".format(",".join(tools.keys())))
parser.add_argument('--cdb','-c', action='store', dest='cdb', metavar='f',
                    default='compile_commands.json',
                    help='Full name with path to compile_commands.json one wishes to process')
parser.add_argument('--tool','-t', action='store', dest='tool',
                    default='clang-tidy', metavar='toolname',
                    help='Tool to use to process cdb file')
xml = parser.add_mutually_exclusive_group()
xml.add_argument('--xml', dest='xml', action='store_true',
                    help='If the tool allows, generate report in XML, defaults to plaintext')
...
allow_dupes.add_argument('--dupes', dest='allow_dupes', action='store_true',
...
parser.add_argument('--jobs','-j', type=int,  dest='jobs', action='store',
                    default=0, metavar='N',
                    help='If chosen tool allows, use this many parallel processes. 0 for automatic detection, otherwise, limit to N')
parser.add_argument('--commit-a', type=str,  dest='commit-a', action='store',
                    default=None, metavar='N',
                    help='Limit scanning to particular commit^')
parser.add_argument('--commit-b', type=str,  dest='commit-b', action='store',
                    default=None, metavar='N',
                    help='If specified, scan between commit-a and commit-b, otherwise commit-a is commit-a^')
parser.add_argument('file', nargs='*', type=os.path.abspath,
...
if os.path.isfile(args.cdb):
    ...
    if cdb != None:
        if args.commit-a != None:
            cdb = remove_untouched_files(cdb, (args.commit-a, args.commit-b))
